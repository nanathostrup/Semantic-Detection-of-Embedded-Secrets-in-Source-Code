...
namespace Project.SecretDetection.Semantics{
    ...
    {
        public Dictionary<SyntaxToken, List<SyntaxToken>> initDataflow3(List<SyntaxTree> trees, List<SyntaxToken> idTokens)
        {
                    ...
                    var matches = root.DescendantNodes()
                        .Where(node =>
                        {
                            ...
                            .Select(node =>
                            {
                                return node switch
                                {
                                    IdentifierNameSyntax ins => ins.Identifier,
                                    VariableDeclaratorSyntax vds => vds.Identifier,
                                    ParameterSyntax ps => ps.Identifier,
                                    _ => default
                                };
                            })
                ...
                SyntaxNode node)
            {
                return node switch
                {
                    ...
                    MethodDeclarationSyntax m =>
                        model.GetDeclaredSymbol(m),

                    PropertyDeclarationSyntax p =>
                        model.GetDeclaredSymbol(p),

                    IdentifierNameSyntax i =>
                        model.GetSymbolInfo(i).Symbol,
